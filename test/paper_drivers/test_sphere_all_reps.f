      ...
      real *8, allocatable :: rhs(:),sigma(:),u(:)
     ...
     1       srcvals,srccoefs,ifplot,fname)
      ...
      allocate(rhs(npts),u(npts),sigma(npts))
     ...
     1  srcvals,eps,numit,rhs,irep,eps_gmres,niter,errs,rres,sigma,u)
      ...
      ru = -1.0d0/(nn+0.0d0)/(nn+1.0d0)
      rsigma = (2*nn+1.0d0)*ru
      ...
      do i=1,npts
        ra = ra + rhs(i)**2*wts(i)
        erru = erru + (u(i) - ru*rhs(i))**2*wts(i)
        errsig = errsig + (sigma(i)-rsigma*rhs(i))**2*wts(i)
      enddo
      erru = sqrt(erru/ra)
