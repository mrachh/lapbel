      ...
      real *8, allocatable :: sigma(:),jvec(:,:),rn_jvec(:)
     ...
     1       srcvals,srccoefs,ifplot,fname)
      ...
      allocate(sigma(npts),jvec(3,npts),rn_jvec(npts))
...
c
c
      call lpcomp_curl_s_tan_grad(npatches,norders,ixyzs,iptype,
     1  npts,srccoefs,srcvals,eps,sigma,irep,jvec)

      
      rrn = 0
      if(irep.eq.1) rrn = -(nn+0.0d0)*(nn+1.0d0)/(2*nn+1.0d0)**2
      if(irep.eq.3) rrn = -(nn+0.0d0)*(nn+1.0d0)/(2*nn+1.0d0)**3
      erru = 0
      ru  =0
      do i=1,npts
        rn_jvec(i) = jvec(1,i)*srcvals(10,i) + 
     1      jvec(2,i)*srcvals(11,i) + jvec(3,i)*srcvals(12,i)
cc        if(i.le.5) print *, rn_jvec(i),sigma(i),rn_jvec(i)/sigma(i),rrn
        ru = ru + sigma(i)**2*wts(i)
        erru = erru + (rn_jvec(i)-rrn*sigma(i))**2*wts(i)
      enddo
      erru = sqrt(erru/ru)
