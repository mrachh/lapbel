...
c
     ...
     2    nnz,row_ptr,col_ind,rfac,novers,ixyzso)
      ...
      allocate(srcover(12,npts_over),wover(npts_over),rrhso(npts_over))
     ...
     1        srcover,wover)
      ...
      call oversample_fun_surf(1,npatches,norders,ixyzs,iptype,npts,
     1  rrhs,novers,ixyzso,npts_over,rrhso)
...
C$OMP END PARALLEL DO    
      ...
      call dgetnearquad_ggq_guru(npatches,norders,ixyzs,iptype,npts,
     1  srccoefs,srcvals,ndtarg,npts,srcvals,ipatch_id,uvs_targ,
     2  eps,ipv,fker,ndd,dpars,ndz,zpars,ndi,ipars,nnz,
     3  row_ptr,col_ind,iquad,rfac0,nquad,wnear)

      call prinf('finished generating near quadrature correction*',i,0)

c
c  test layer potential evaluation at few points
c
        ...
        do ipatch=1,npatches
          ...
          if(ifnear.eq.0) then
            ...
            do l=1,npolso
              val = 0
              lpt = l+istart-1
              call fker(srcover(1,lpt),12,srcvals(1,i),ndd,dpars,ndz,
     1           zpars,ndi,ipars,val)
              pot(i) = pot(i) + val*wover(lpt)*rrhso(lpt)
            enddo
          endif

          if(ifnear.eq.1) then
            jquadstart = iquad(iind)
            jstart = ixyzs(ipatch)
            npols = ixyzs(ipatch+1)-ixyzs(ipatch)
            do l=1,npols
              pot(i) = pot(i) + wnear(jquadstart+l-1)*rrhs(jstart+l-1)
            enddo
          endif
        enddo
        pot(i) = pot(i)/4/pi
      ...
      enddo
      erra = sqrt(erra/ra)
      call prin2('error in application at 10 points=*',erra,1)

      call prin2('pot=*',pot,10)
      call prin2('rrhs=*',rrhs,10)
