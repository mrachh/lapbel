...
c     1       srcvals,srccoefs,ifplot,fname)
      ...
      fname = '../../geometries/stell_150_o05.go3'
...
C$OMP END PARALLEL DO      
     ...
     1   srccoefs,srcvals,V)
      ...
      rr = ((1.0d0)/(4*(2*nn+1.0d0)**2)) 
      do i=1,npts
        erra=  erra + ((V(1,i))**2)*wts(i)
        erra=  erra + ((V(2,i))**2)*wts(i)
        erra=  erra + ((V(3,i))**2)*wts(i) 
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of V=*',erra,1)
     ...
     1   srccoefs,srcvals,V,F)
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of F=*',erra,1)

      erra = 0
      ra = 0
      rr = 0 
      do i=1,npts
        rr=rr+F(1,i)*srcvals(10,i)
        rr=rr+F(2,i)*srcvals(11,i)
        rr=rr+F(3,i)*srcvals(12,i)
 
        erra=  erra + ((rr)**2)*wts(i)
        ra = ra + (Ft(1,i))**2*wts(i)
        ra = ra + (Ft(2,i))**2*wts(i)
        ra = ra + (Ft(3,i))**2*wts(i)
      ...
      enddo
      erra = sqrt(erra)
      call prin2('normal comp of tangential proj=*',erra,1)
     ...
     1   srccoefs,srcvals,F,nF)
      ...
      rr = 0
      do i=1,npts
        erra=  erra + ((nF(1,i))**2)*wts(i)
        erra=  erra + ((nF(2,i))**2)*wts(i)
        erra=  erra + ((nF(3,i))**2)*wts(i) 
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of nxF*',erra,1)
      ...
      rr = 0.0d0 
      do i=1,npts
        rr=rr+nF(1,i)*srcvals(10,i)
        rr=rr+nF(2,i)*srcvals(11,i)
        rr=rr+nF(3,i)*srcvals(12,i)
      ...
      enddo
      call prin2('normal comp of nF=*',erra,1)
     ...
     1   srccoefs,srcvals,F,rrhs1)
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of rrhs1 =*',erra,1)
     ...
     1   srccoefs,srcvals,nF,rrhs2)
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of rrhs2 =*',erra,1)
...
c    Surface integral should be zero
      ...
      do i=1,npts
        Wu = Wu + rrhs1(i)*wts(i) 
      enddo
      call prin2('stell surface integral of rrhs1=*',Wu,1)
...
c    Surface integral should be zero
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of alpha =*',erra,1)
...
c    Surface integral should be zero
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of beta =*',erra,1)
     ...
     1   srccoefs,srcvals,sgbeta,nsgbeta)
      ...
      enddo
      erra = sqrt(erra)
      call prin2('L2 norm of H=*',erra,1)
...
c
     ...
     1   npts,srccoefs,srcvals,V) 
      ...
      l(3) = 10.0d0
      do i=1,npts
        lcross(1,i) = l(2)*srcvals(3,i)-l(3)*srcvals(2,i) 
        lcross(2,i) = l(3)*srcvals(1,i)-l(1)*srcvals(3,i) 
        lcross(3,i) = l(1)*srcvals(2,i)-l(2)*srcvals(1,i) 
      enddo


      do i=1,npts
        nrm = srcvals(1,i)**2+srcvals(2,i)**2+srcvals(3,i)**2
        nrm = nrm**(1.5)
