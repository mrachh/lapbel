...
c
      ...
      subroutine getnearquad_lap_bel3(npatches,norders,
...
c
c
      subroutine lpcomp_lap_bel3(npatches,norders,ixyzs,
...
c
      call lpcomp_lap_bel_addsub3(npatches,norders,ixyzs,
...
c
c
      subroutine lpcomp_lap_bel_addsub3(npatches,norders,ixyzs,
     1   iptype,npts,srccoefs,srcvals,
...
C$OMP$PRIVATE(ctmp2,nss,l,jstart,ii,val,npover,vgrad,nvgrad)
     ...
     1           vgrad(3)*srcvals(12,i)
        ...
        normalgrad(i) = normalgrad(i) - nvgrad
...
c      normalgrad is calculating S' now. Use it as density
c      
c
...
c            
      ...
      call oversample_fun_surf(1,npatches,norders,ixyzs,iptype, 
     1    npts,normalgrad,novers,ixyzso,ns,sigmaover)
...
C$OMP$PRIVATE(jstart,pottmp,npols)
      do i=1,ntarg
        do j=row_ptr(i),row_ptr(i+1)-1
          ...
          do l=1,npols
            pot2(i) = pot2(i) + 
     1          wnear(2*nquad+jquadstart+l-1)*normalgrad(jstart+l-1)
          enddo
        enddo
      enddo
C$OMP END PARALLEL DO
...
C$OMP$PRIVATE(jstart,pottmp,npols)
      do i=1,ntarg
        do j=row_ptr(i),row_ptr(i+1)-1
          ...
          do l=1,npols
            normalgrad(i) = normalgrad(i) + 
     1          wnear(2*nquad+jquadstart+l-1)*pot1(jstart+l-1)
          enddo
        enddo
      enddo
C$OMP END PARALLEL DO
...
c     HS'S calculated in pot3
c
c     Now calculating (S'' + D')S
c     
c
      call oversample_fun_surf(1,npatches,norders,ixyzs,iptype, 
     1    npts,sigma,novers,ixyzso,ns,sigmaover)
...
c      call prin2('sigma=*',sigma,24)
c      call prin2('sigmaover=*',sigmaover,24)
...
C$OMP$PRIVATE(jstart,pottmp,npols,l)
     ...
     1          wnear(3*nquad+jquadstart+l-1)*pot1(jstart+l-1)
          enddo
        enddo
      enddo
C$OMP END PARALLEL DO
...
C$OMP$PRIVATE(nvgrad,nvhess) 
     ...
     1                  vhess(3)*srcvals(12,i)*srcvals(12,i) 
        ...
        pot4(i) = ngradphess(i) - nvgrad - nvhess
        deallocate(srctmp2,ctmp2,dtmp2)
      enddo

c      call prin2('pot1=*',pot1,24)

c      print *, "Subtraction done"

c     pot4 has (S'' + D')S
c
c     -----Now calculate WS^2------ 
      call oversample_fun_surf(1,npatches,norders,ixyzs,iptype, 
     1    npts,sigma,novers,ixyzso,ns,sigmaover)
...
c      pot5 has S^2, now apply W
      call get_qwts(npatches,norders,ixyzs,iptype,npts,srcvals,wts)

      rint = 0

      do i=1,npts
        rint  = rint + pot5(i)*wts(i) 
      enddo
...
c
c        
      subroutine lap_bel_solver3(npatches,norders,ixyzs,
...
c
      dpars(1) = 1.0d0/4/pi
      dpars(2) = 0
c      call lpcomp_lap_comb_dir_addsub(npatches,norders,ixyzs,
c     1  iptype,npts,srccoefs,srcvals,ndtarg,npts,targs,eps,
c     2  dpars,nnz,row_ptr,col_ind,iquad,nquad,wnear(0*nquad+1),
c     3  rhs,novers,npts_over,ixyzso,srcover,wover,rhs2)
c      erra = 0
c      ra = 0
c      do i=1,npts
c        erra = erra + (rhs(i)/5.0d0-rhs2(i))**2
c        ra = ra + rhs2(i)**2
c      enddo
c      erra = sqrt(erra/ra)
c      call prin2('error in rhs=*',erra,1)
